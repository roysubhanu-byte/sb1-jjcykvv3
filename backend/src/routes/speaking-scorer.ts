...
router.post('/score', async (req, res) => {
  try {
    ...
    if (!transcript || !audioFeatures) return res.status(400).json({ error: 'Missing transcript or audioFeatures' });
...

{
  ...
  "improvements": string[]
    ...
    if (!json) return res.status(500).json({ error: 'No response from AI' });
